...
class MQTTBridge:
    ...
    def _create_mqtt_client(self) -> mqtt.Client | None:
        ...
        try:
            mqtt_client.connect(host=self._fiber_config.mqtt.host, port=int(
                self._fiber_config.mqtt.port))
    ...
    def send_json(self, topic: str, obj: dict | None, prefix='fiber/') -> None:
        logger.debug(
            f'Sending JSON object message on {prefix}{self._fiber_id}{topic} with object {obj}')
        if self.mqtt_client:
            try:
                if obj is not None:
                    obj = json.dumps(obj)
