...
pub fn all_settable_keys() -> Vec<&'static str> {
    // The 16 numbered alarm slots are settable but excluded from the default
    // read: requesting all of them in one GetParam overflows the sticker's
    // request array (it replies bad_request "array overflow"). The default read
    // stays the small scalar set; alarm slots are read explicitly when needed.
    SETTABLE
        .iter()
        .map(|(k, _, _)| *k)
...
mod tests {
    ...
    fn all_settable_keys_excludes_alarm_slots() {
        // Numbered alarm slots must not be in the default read (they overflow a
        // single GetParam); the scalar params, incl. the alarm scalars, stay.
